...
if(mysqli_num_rows($sql_verifica) >= 1 && $cpf != 0){
  echo "<script language='javascript'>window.alert('Já existe um cliente cadastrado com as informações apresentadas!');</script>";
}else{
	...
	if($sql_inseri == ''){
		...
		echo "<strong>Cliente cadastrado com sucesso!</strong><br><br><em>Pressione F5 para mesclar a operação.</em>";
		die;
	}
  ...
  <table width="900" border="0">
    <tr>
      <td colspan="6" align="center"><h1 style="font:20px Arial, Helvetica, sans-serif;"><strong>ADICIONAR NOVO CLIENTE</strong></h1>
        <hr />
<h2 style="border:1px solid #000; padding:10px; color:#F00; text-align:center; border-radius:5px; width:300px;">Código do novo cliente:<br /><? echo $code = rand()+date("s")+date("Y")+date("m")*date("s"); ?></h2></td> <input type="hidden" name="code" value="<? echo $code; ?>" />
    </tr>
    <tr>
